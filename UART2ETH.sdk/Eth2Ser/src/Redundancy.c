...
void Send_Port_status_to_Slave_Board(struct slave_port_status* slave_port_status, struct uart_port_features* uart_port_features){
	...
	int indx = 0;
	int num = (Max_Num_of_Serial_Ports-1);
	for(int i=0; i<M2S_Preamble_Size ; i++){
		buffer[i] = 'M' + i;
	}
	...
	for(int p=num; p>=0 ; p--){
		...
		port_status = port_status | slave_port_status->serial_port_enable[p];

	}
	buffer[M2S_Preamble_Size+1] = (port_status >> 8) & 0x00FF;
	buffer[M2S_Preamble_Size+2] = (port_status) & 0x00FF;
	buffer[M2S_Preamble_Size+3] = slave_port_status->ethernet_port_enable;
