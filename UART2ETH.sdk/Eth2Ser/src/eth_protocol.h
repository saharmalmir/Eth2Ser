...
#define eth_circular_FIFO_size							20000
#define Max_Number_Eh_packet_in_Queue					10
#define Eth_Rcv_Buff_Size     	    					15000
...
struct Eth_Header{
	...
	int protocol_ID;
	struct creat_protocol creat_protocol;
	int sequence_number;
	bool packet_type;
	int more_fragment;
	u8 bodysize[2];
...
struct new_packet_spec{
	int Wr_Index;
	int Rd_Index;
	int length[Max_Number_Eh_packet_in_Queue];
};

struct process_flags{
	bool packet_process_ended;
	bool whole_data_recvd_processed;
	bool check_Protocol;
	bool send_raw_data;
};
...
struct Ethernet_Port_spec{
	...
	bool packet_process_ended;
	bool eth_packet_process_ended;
	bool input_data_process_finished;
	struct circulr_buffer circulr_buffer;
	struct circulr_buffer buffer_Index;
	struct new_packet_spec new_packet_spec;
...
void Assign_Ethernet_pkt_features_from_received_pkt(struct Ethernet_Port_spec* Ethernet_Port_spec , u8* Ethernet_Temp_Rcv_Buff ,  struct referenced_protocols* referenced_protocols , struct Diagnostics* Diagnostics);
