...
void GLCD_Init()
{
    ...
    //GlcdCtrlBusDirnReg &= ~((1<<LCD_IO_RS)|(1<<LCD_IO_RW)|(1<<LCD_IO_EN)|(1<<LCD_IO_CS1)|(1<<LCD_IO_CS2)); // Configure the Ctrl pins as output
...
void GLCD_HorizontalGraph(uint8_t var_barGraphNumber_u8, uint8_t var_percentageValue_u8)
{
    ...
    if((var_barGraphNumber_u8 < C_MaxBarGraphs_U8) && (var_percentageValue_u8<=100))
    {
        ...
        perventile = var_percentageValue_u8;
        GLCD_DisplayNumber(C_DECIMAL_U8,perventile,3);
    }
}
#endif
...
{
	glcd_BusyCheck();
	GLCD_Write_Data_Bus(cmd);//GlcdDataBus = cmd;
	GLCD_Clear_Bit(LCD_IO_RS);	  // Select the Command Register by pulling LCD_IO_RS LOW
	GLCD_Clear_Bit(LCD_IO_RW);	   // Select the Write operation by pulling LCD_IO_RW LOW
	GLCD_Set_Bit(LCD_IO_EN);		   // Send a High-to-Low Pulse at Enable Pin
	usleep(1);
	GLCD_Clear_Bit(LCD_IO_EN);
}
...
void glcd_DataWrite( uint8_t dat)
{
	...
	glcd_BusyCheck();
	uint8_t data = dat ^ GLCD.Invertdisplay;
	GLCD_Write_Data_Bus(data); //GlcdDataBus = dat ^ GLCD.Invertdisplay;
	GLCD_Set_Bit(LCD_IO_RS);							// Select the Data Register by pulling LCD_IO_RS LOW
	GLCD_Clear_Bit(LCD_IO_RW);						// Select the Write operation by pulling LCD_IO_RW LOW
	GLCD_Set_Bit(LCD_IO_EN);						   // Send a High-to-Low Pulse at Enable Pin
	usleep(1);
	GLCD_Clear_Bit(LCD_IO_EN);
...
{
	GLCD_Set_Bit(LCD_IO_CS1);
	GLCD_Clear_Bit(LCD_IO_CS2);
}


void glcd_SelectPage1()
{
	GLCD_Set_Bit(LCD_IO_CS2);
	GLCD_Clear_Bit(LCD_IO_CS1);
}
...
{	
	...
	GLCD_Clear_Bit(LCD_IO_RS);           // Select the Command Register by pulling LCD_IO_RS LOW
	GLCD_Set_Bit(LCD_IO_RW);             // Select the Read Operation for busy flag by setting LCD_IO_RW
	do
	{

		GLCD_Clear_Bit(LCD_IO_EN);             // Send a High-to-Low Pulse at Enable Pin
		usleep(10);
		GLCD_Set_Bit(LCD_IO_EN);
		usleep(10);
